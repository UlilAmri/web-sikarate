  ...
  const fetchData = async () => {
    ...
    try {
      const [resBerita, resKategori] = await Promise.all([
        axios.get("https://api-sikarateka.mataramkota.go.id/blog/", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get("https://api-sikarateka.mataramkota.go.id/kategori/", {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
...
const handleSubmit = async (e) => {
  ...
  try {
    if (isEdit && form.id_artikel) {
      ...
      await axios.put(
        `https://api-sikarateka.mataramkota.go.id/blog/${form.id_artikel}`,
        {
      ...
      // Tambah artikel baru
      await axios.post(
        "https://api-sikarateka.mataramkota.go.id/blog/",
        {
          id_kategori: parseInt(form.id_kategori),
          judul: form.judul,
          isi: form.isi,
  ...
  const handleDelete = async (id_artikel) => {
    ...
    if (window.confirm("Apakah Anda yakin ingin menghapus berita ini?")) {
      try {
        await axios.delete(`https://api-sikarateka.mataramkota.go.id/blog/${id_artikel}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
